...
//    }
    ...
    public static <E> E getDataFromMap(Map<String, Object> map, String key){
    ...
    private static String mobileAppUserAgent  = null;
    public static void setMobileAppUserAgent(Context context){
        mobileAppUserAgent = InstaApiUtil.getUserAgent(context);
    }

    public final static String getMobileAppUserAgent() {
        if(mobileAppUserAgent != null){
            return mobileAppUserAgent;
        }else{
            int i = 640;
            int j = 2392;
            int k = 1440;
...
//        return "Instagram 7.16.0 Android (18/4.3; 320dpi; 720x1280; " + Build.MANUFACTURER + "; " + Build.MODEL + "; armani; qcom; en_US)";
        ...
        int i = (int)(metrics.density * 160.0F); //640
        int j = metrics.heightPixels; //2392
        int k = metrics.widthPixels; //1440
        return String.format("%s Android (%s/%s; %s; %s; %s; %s; %s; %s; %s_%s)", new Object[] { "Instagram 10.15.0", Build.VERSION.RELEASE, Build.VERSION.SDK, i + "dpi", j + "x" + k, Build.MODEL, Build.MANUFACTURER, Build.BRAND, Build.DEVICE, Locale.getDefault().getLanguage(), Locale.getDefault().getCountry() });
    }
...
//        }
    ...
    public final static String localeToBcp47Language(Locale loc) {
